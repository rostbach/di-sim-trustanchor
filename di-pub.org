...
#+DATE: 24.10.2015 {12., 8, 3.10., 28.,2.9., 20.,18.8.,27.7,11.5,7.5.to the github,28.4, 16.4, 13.4.2015 new-framing, 4.4, 27.3,  20.3, 7.3)
...
#+LATEX_HEADER: \thesistype{draft-24.10.2015 Master of Science thesis}
...
** Homenet without PKI 
 Behringer continues, that the end devices would not yet get full AuthZ,
but only a ticket, with which the end device may request for service tickets.
with that ticket, the device can apply for access to change management
...
Kerberos style authentication network, where time-limited tickets (tokens) for
...
** Trust anchoring shortly explained
...
unknown device.  
# If the keys are stored in SIM as they here are, an external operator support is needed.
# # this is not PKI
...
located on diverse place, as can be seen in Section [[#sec:scenarios]].
# ### 
...
The steps are numbered  in Figure[[fig:scenario-I]] and explained in detail.
...
2. The Cloud send the changes to the Local Controller (phone) (2).
...
now AuthZ is checked from the Cloud by CPE instead of MNO. If there is no connection to
...
#+end_src
#+CAPTION: Scenario IV. In the future CPE could be attached direct to the Cloud and AuthN comes from MNO
...
but CPE has neither trust nor roles.
...
** Alternative management models (-> to discussion and speculations)
...
socket to an external hlr_auc_gw program."
#+END_VERSE
...
** Extension of the WPA2 supplicant to the real smartphone
...
need to extend the number of SSID's. Last, we discuss and speculate on 
...
methods. All these methods allows the separation of the user in Authentication Server.
...
They may consult local RADIUS server, whether there currently is an
authenticated smartphone present, which indicated that changes coming
from the management network would be allowed. Smartphone also could
know the login credentials to devices through earlier received
...
In the dDesign chapter it was questioned whether the proxying RADIUS server
...
Altering or addition of  RADIUS messages can choose from many

...
\thesistype{draft-3.10.2015 Master of Science thesis}
...
justifies the management and therefore the changes to be allowed, if the smartphone user is currently logged in to the management network.
...
It is not known for the author which type represent the majority of
change requests.
...
has not been done yet. The scenario can be any of I-IV, but no trust nor roles are present in CPE.
...
There is also fraudulent Authenticator problem, which is an attack
against both integrity and privacy.  The Authenticator may present
...
\subsection{Fraudulent Authenticator}
\label{sec-6-4-7}

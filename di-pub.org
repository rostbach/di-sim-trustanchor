...
anchoring and AuthN.  Bootstrapping protocols in general are used to
bring the first trust anchor in an environment and use that anchor to
attach other devices to the same trust circle.  A draft from
Behringer\cite{draft-behringer-bootstrap} proposes, that one device is
first chosen as a trust anchor and trust is built upon that anchor.
...
For that, AuthN of the trust anchor device would happen at Cloud but
it is done by checking device's Vendor certificates. This anchored
...
would apply for certificates from to get under the same trust circle.
 ...
 Behringer proposes that
  This AuthN would not yet give full AuthZ, but a

...
\thesistype{draft-28.9.2015 Master of Science thesis}
...
Access-Accept or Access-Deny RADIUS message. Accept triggers
802.1x protocol to open a virtual port in AP and let the WPA2 process
continue in exchanging WPA2 session keys. 
...
Even, when AP has authenticated the smart home, managed devices still 
...
but this time there is service provider between CPE and MNO, so AA is fully outsourced:
...
EAP-SIM on factory software (Symbian). 
...
server, and hlr\_auc\_gw programs were used.  HSS would replace HLR in
3G/UMTS networks.  The version of HostAP used in the tests was 2.2,
...
firmware was used instead of \emph{hostapd}'s AP. OpenWRT AP works as a
RADIUS client connecting to RADIUS server still provided by \emph{hostapd}.
OpwnWRT will not try to open EAP-messages or need to know about them;
it just encapsulates them into RADIUS packet.
RADIUS server's configuration file can be seen in Appendix \ref{app:radius-conf}.


Laptop's role was therefore physically split-brain representing all
those components: it asked from itself for AA. 
...
802.1X, RADIUS and HLR is shown below.
...
135 & 15:57:17.9945 & AP-wired & RADIUS-srv & RADIUS & 272 & Access-Request(1) (id=163, l=230)\\
 &  & RADIUS-srv & HLR-AuC gw & socket &  & SIM-REQ-AUTH <IMSI> 3\\
 &  & HLR-AuC gw & RADIUS-srv & socket &  & SIM-RESP-AUTH <IMSI>, 3 triplets\\

...
model is explained in Chapter \ref{sec-1}.  Chapter \ref{sec-2}
describes security in current home network architecture and 
practices for configuring it.  Chapter \ref{sec-3} discusses methods
...
operator involved.  Those experiments are described in Chapter \ref{sec-4}.
Results are discussed in Chapter \ref{sec-5} and Chapter \ref{sec-6} concludes the
thesis.
...
located on diverse places, as can be seen in Section \ref{sec-3-2}.
...
In the design chapter (Chapter \ref{sec-3}) it was questioned whether the proxying RADIUS server
...
\ref{sec-3}, where there is local authentication server in home network.

...
circle. Behringer proposes that AuthN would happen likewise
first at cloud provider's end but it is done by checking device's Vendor
certificates. 
This AuthN would not yet give full AuthZ, but a ticket, with which
the end device may request for service tickets. That similar to Kerberos, which is based on Needham-Schroeder
protocol. The device applies with that ticket for access to change
management service from CPE.

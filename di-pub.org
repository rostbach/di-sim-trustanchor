...
The AA components AuthN and AuthZ can be found in diverse location combinations. Here, the AuthN component 
...
Authenticator on the other hand must lie at home, and it always 
gives the final decision about the access.

If the AuthZ decision is made on remote, 3rd party AuthZ server, 
then that server needs either to have local data or access to 

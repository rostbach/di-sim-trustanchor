...
#+LATEX_HEADER: \thesistype{draft-11.8.2015 Master of Science thesis}
...
a model where the management is achieved through a service in a cloud. The
...
file:~/gitdocs/di/images/localcontroller.png
...
end (not shown in the Figure[[fig:eap-layers]]) of EAP messaging,
...
#   Figure [[fig:eap-sim-full]] describes the EAP-SIM authentication
# sequence.  
...
# From the Figure[[fig:eap-sim-full]] we can see, that IMSI, which is client's identity, is
...
/unique/, so they can only be considered as strong passwords and they
...
home network has received a trust anchor.  Before fully explaining our
chosen method, we introduce some alternative approaches for a trust
anchor, which is part of bootstrapping.  Bootstrapping is needed
because although the smartphone and MNO already trust each other, the
trust between the smartphone and AP, and thus the management network
at home, is not yet clear in the beginning.

On its simplest, the smartphone connects with a Wi-Fi link to an
access point (AP) in the home network and authenticates with SIM-card.
# AP functions here as an Authenticator. 
That brings a trust relationship between smartphone (local controller)
and home network (managed devices) and configuration changes can
...
# The smartphone will approve changes for homenet and is part of
...
# what happens when accessing homenet?
...
1. Smartphone connects wireless AP 
2. AP, as RADIUS client, connects ROUTER (with Lock-and-key) 

Now connection,  from smartphone to configured router is open and 
smartphone may try to login there.
 
2. smartphone use telnet (or ssh) to login to the ROUTER.
3. ROUTER(as RADIUS client) checks AA from AP(working as
   proxy) 
4. AP knows what? It wants to give access, but can it map this request to
   earlier facts?
...
WPA2-Supplicant to AP (see Figure[[fig:eap-sim-full]], message 2.)

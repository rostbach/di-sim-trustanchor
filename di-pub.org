...
separate segments might not belong to the home owner anymore.
So there will be a need to delegate the management of the home network to 
multiple owners. 
...
often authenticated only by simple password and physical precence near
the configured network device. What then, when the person is note
present physically, but tries to connect remotely?  If external help
would be bought, one needs to have some form of control in allowing
only authorized operators to configure those devices.
...
have a sensor and controller network physically using  the home network, but
...
The main focus here is on authentication and authorization part of
...
Human aspect and usability also are important, although the focus will
not be there. The proposed model should nevertheless require less
effort distributing user credentials, finding the place where they can
be inserted, and ensuring that they are written correctly than the
...
the hardware and the applications mostly support it, but it still is not yet
widely used.  Could there exist a light method to use the SIM?  

The trust can be derived from facts that already are known.
The place, where a trust is not anymore derived or built upon other 
fact but assumed to be present, is called a trust anchor.
[OR: The trust anchor is then the fact, state or place, where derivation is
done no more, but accepted per se. ]
Combining existing techniques, this thesis presents one possible way to bind the
...
Different authentication methods can be used with EAP,  for example hashed
...
+-----+--------------------+  |                                  |    
      ...
      |                       |                                  |
#+end_src
#+CAPTION: EAP-SIM simplified sequence diagram, based on RFC4186

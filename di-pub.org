...
who needs them and server providing them.
# client and Authenticator server.

Here, only first two A's are used and later described as AA
...
control. Ports are on physical layer, not on Layer-4 such as TCP/UDP ports.
...
into wire. In wired net, RADIUS encapsulates EAP
messages.

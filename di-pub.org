...
Scenario V represents the case when nothing has been configured in network, there are no 
admin users, and APs are set to factory default. The scenario V 
needs pre-configuring and bootstrapping and after that the scenario 
can be any of scenarios I-IV, but CPE  has first neither trust nor roles. Bootstrapping is discussed in 

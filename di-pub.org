...
#+LATEX_HEADER: \thesistype{draft-27.10.2015 Master of Science thesis}
...
which is the identity of the phone's SIM.
Light than full, fast re-authentication would use temporal identity
TMSI
and it is shown in Figure[[fig:scenario-IV]]. TMSI can change
...
the right mapping to IMSI authorization so this works even when
there is no internet connectivity,
but Full authentication would not work here then because of missing MNO.
...
# They are needed, if the smart phone makes a re-authentication 
# message with it 
...
#+end_src
#+CAPTION: In the future, CPE could be attached direct to the Cloud and AuthN comes from MNO
...
token with a validity time and had to present that when accessing
services, here the management network. 


As can be seen, no public key infrastructure (PKI) is used before setup
phase is ready.

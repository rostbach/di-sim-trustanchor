...
enterprises and nation wide community federations as the Authenticator
in 802.1X may consult external RADIUS server for authentication
requests.

Federations started as local WLAN groups
such  as SparkNet or LangatonTampere 
which had their own uniform SSID (Service Set IDentifier, published
network name). Later, those groups agreed to allow network visiting
(roaming) from local group to another and so federations were born.

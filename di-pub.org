...
2) Changes are delivered from cloud to smartphone, which forwards them
...
\lstset{basicstyle=\ttfamily\tiny,columns=fixed,emph={"1232010000000000",musta,0,AVP},emphstyle=\color{red}\textbf}
\renewcommand{\lstlistingname}{Capture}
\begin{lstlisting}[
  language=Python,
...
IMSI, but needs to get AuthZ information. 
It can get AuthZ either from the remote operator which would be easier for the
Authenticator or there might be a proxying RADIUS, which inserts the AuthZ
information into ACCESS-ACCEPT packet. The latter has issues with
temporal identities. 
...
because it knows for sure only originating server. 

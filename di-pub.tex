...
\thesistype{draft-14a.11.2015 Master of Science thesis}
...
Access to an end device must be provided from outside, a trustful operator must be hired and login credentials shared.n
...
A theory, how SIM-authentication can be used at home is presented and a simulated environment
...
As results, it is shown, that 
getting in to the management network at home can be carried out via EAP-SIM authentication.
...
dependency to the mobile operator. Additionally, there will remain challenges
...
controller is assumpted to already be trusted.
...
model is explained in Chapter \ref{sec-1}.  Chapter \ref{sec-1-7}
describes security in current home network architecture and 
practices for configuring it.  Chapter \ref{sec-1-10} discusses methods
...
operator involved.  Those experiments are described in Chapter \ref{sec-1-17}.
Results are discussed in Chapter \ref{sec-1-20} and Chapter \ref{sec-1-27} concludes the
thesis.
...
\begin{itemize}
\item Authentication, Authorization, and Trust
\end{itemize}
...
\begin{itemize}
\item Home network architecture
\end{itemize}
...
\begin{itemize}
\item Design of home network trust anchor and separation of change management
\end{itemize}
...
located on diverse places, as can be seen in Section \ref{sec-1-12}.
...
Physical devices used in the tests  were three smartphones, a Wi-Fi accesspoint, and a laptop.
...
(Figure\ref{fig:eap-layers}, Chapter \ref{sec-1}) from 802.1X's 
...
\begin{itemize}
\item Analysis, Results and Discussion
\end{itemize}
...
In the design chapter (Chapter \ref{sec-1-10}) it was questioned whether the proxying RADIUS server
...
\ref{sec-1-10}, where there is local authentication server in home network.

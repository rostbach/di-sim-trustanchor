...
would get positive AuthN result, the AuthZ still would deny him the access.
 When we think, that local role cache only involves few numbers, this is tolerable burden.

there are not many users to administer.   external MNO
can not authorize 
...
use temporal identity TMSI, is lighter in operation than full

...
#+DATE: 20.8.2015  {18.8,27.7,11.5,7.5.to the github,28.4, 16.4, 13.4.2015 new-framing, 4.4, 27.3,  20.3, 7.3)
...
#+LATEX_HEADER: \thesistype{draft-20.8.2015 Master of Science thesis}
...
# physical presence
Remote management eliminates consultant's 
...
safely. 
# So there will be a need to delegate the management of
# the home network to multiple owners. 
...
See Figure[[fig:localcontroller]] for design of this architecture.
...
[OR: /The trust anchor is then the fact, state or place,
...
The thesis is structured as follows: in Chapter [[#cha:aaa]] the
authentication--authorization model is explained.  Chapter [[#cha:management]] describes
...
AuthN. Comparing to software based certificates, while stronger than regular passwords,
...
the middle, and MAP/SS7, which is cellular telephony
...
4 (Figure[[fig:eap-sim-radius]]). The client later checks in the process
...
Smartphone                           Authenticator (AP)
  ...
  +-------------------------------------------->|
  |                                             |
  |                        2.AP Request/Identity|
  |<--------------------------------------------+
  |                                             |
  |3.EAP Response/Identity(IMSI) [later X]      |
  +-------------------------------------------->|                                     RADIUS
  |                                             |                               Authentication Server
  ...
  |                                             |                                     |
+-+--------------------------------------+      |                                     |
|run GSM algorithms, verify H(RAND) with |      |                                     |
|NONCE, derive sess. key Kc∗, decrypt and|      |                                     |
...
separate home networks, which need authentication service.
# clients.
# picture
...
#+begin_src ditaa :file scenI.png :cmdline -E -r -s 4
  ...
  +=------------------------------------+
#+end_src
#+CAPTION: Scenario I with 3 separate domains: BaaS, MNO and home net
#+NAME:   fig:scenario-I
#+RESULTS:
[[file:scenI.png]]
...
8. IF ACCEPTed, then the smartphone is both authenticated and authorized (8) and
...
#+end_src
#+CAPTION: Scenario IV, AuthZ from BaaS, AuthN from home network
...
Figure [[fig:cisco-802.1x]] reminds us again how 802.1X works with respects to
Lock-and-key in Figure[[fig:lock-and-view]].
...
getting in the management segment ...
...
AP and /hostapd/ provides only RADIUS server.
...
| 129 | 15:57:17.983047 | AP-802.1x  | smartphone  | EAP      |     23 | Request, Identity                    |
| 130 | 15:57:17.983223 | smartphone | AP-802.1x   | EAP      |     39 | Response, Identity                   |
| 131 | 15:57:17.988716 | AP-wired   | RADIUS-srv  | RADIUS   |    235 | Access-Request(1) (id=162, l=193)    |
| 132 | 15:57:17.988949 | RADIUS-srv | AP-wired    | RADIUS   |    108 | Access-Challenge(11) (id=162, l=66)  |
| 133 | 15:57:17.990884 | AP-802.1x  | smartphone  | EAP      |     38 | Request, GSM Subscriber Identity     |
|     |                 |            |             |          |        | Modules EAP (EAP-SIM)                |
| 134 | 15:57:17.992497 | smartphone | AP-802.1x   | EAP      |     70 | Response, GSM Subscriber Identity    |
|     |                 |            |             |          |        | Modules EAP (EAP-SIM)                |
| 135 | 15:57:17.994547 | AP-wired   | RADIUS-srv  | RADIUS   |    272 | Access-Request(1) (id=163, l=230)    |
| 136 | 15:57:18.002416 | RADIUS-srv | AP-wired    | RADIUS   |    256 | Access-Challenge(11) (id=163, l=214) |
| 137 | 15:57:18.004054 | AP-802.1x  | smartphone  | EAP      |    186 | Request, GSM Subscriber Identity     |
|     |                 |            |             |          |        | Modules EAP (EAP-SIM)                |
| 138 | 15:57:18.004398 | smartphone | AP-802.1x   | EAP      |     46 | Response, GSM Subscriber Identity    |
|     |                 |            |             |          |        | Modules EAP (EAP-SIM)                |
| 139 | 15:57:18.006323 | AP-wired   | RADIUS-srv  | RADIUS   |    248 | Access-Request(1) (id=164, l=206)    |
| 140 | 15:57:18.006513 | RADIUS-srv | AP-wired    | RADIUS   |    202 | Access-Accept(2) (id=164, l=160)     |
| 141 | 15:57:18.011044 | AP-802.1x  | smartphone  | EAP      |     22 | Success                              |
| 142 | 15:57:18.011213 | AP-802.1x  | smartphone  | EAPOL    |    135 | Key (Message 1 of 4)                 |
| 143 | 15:57:18.012353 | smartphone | AP-802.1x   | EAPOL    |    135 | Key (Message 2 of 4)                 |
| 144 | 15:57:18.016163 | AP-802.1x  | smartphone  | EAPOL    |    169 | Key (Message 3 of 4)                 |
| 145 | 15:57:18.016344 | smartphone | AP-802.1x   | EAPOL    |    113 | Key (Message 4 of 4)                 |
#+LATEX: \normalsize

# - flow of messages,  timing,  size, attributes 
  
Even when authentication conversation would not complete fully,
Authenticator still receives a claim of identification from the smartphone. Yet, as
...
We note here, that AP uses version 802.1X-2004 while WPA2-Supplicant
responses with version 802.1X-2001. Here it does not have any
noticeable effect. The identity field's length cannot directly be
seen, but when EAP message length 21 octets is reduced by space
needed for Code(1), ID(1), Lenght(2), and Type(1), it yields 16 octets for
the Identity. Therefore the Identity is not coded as a
numeral but as a string and that brings flexibility as the identity can
include alphabets too. It also minimizes misunderstandings, if context
gets lost.
...
sent to RADIUS server. The capture between AP and Radius server:
...
Identity=``1232010000000000'', because
RADIUS message itself has set User-Name field to very same, ``1232010000000000''. 
Identity is also filled in WPA2-Supplicant configuration file both in identity and credential
section but this might just be syntax issue.
...
Meanwhile, HLR simulator has been started to listen requests from EAP-handler 
...
# #+END_EXAMPLE
# #+LATEX: \normalsize
It receives from internal EAP-handler the AuthN request
(SIM-REQ-AUTH), which in production version would go to real HLR-AuC.
In the message is included the IMSI and parameter "3", which 
indicates, that requester wants three triplets. One triplet equals one
64-bit key used for challenges, so three will make
the key 192 bit long. [theoretically.. see the article smwhere].
...
and if it were, it would also be based heavily on current Perl environment which
...
home network management delegation. The following subsections divide them into
...
man in the middle attack device called IMSI-catcher to fake as being a
base station.  When the smartphone tries to attach to a fake base
...
if that is possible.\cite{imsi-heise}.
...
Most  EAP methods do not provide identity protection
...
RADIUS messaging is also vulnerable. It includes IMSI in clear form 
inside EAP encapsulation. 
Messages include IMSI also in user-name attribute that has been
converted from identity field.
# may be filtered out?
RADIUS runs over UDP, but can be used also with TCP with the RadSec
extension. RADIUS servers' usually support it, but APs don't, so 
still there will be parts, where RADIUS message travels in plain.
...
# 
***  Integrity
Integrity issues were handled in Section[[sec:radius-macs]] describing
...
versions of TLS might in the future require\cite{rfc6614}.
...
external in the cloud. Trust between home network and cloud is searched.
Smartphone lies in the intersection of both domains 
and possess properties to simplify binding of that trust.
SIM card of smartphone, used together with Wi-Fi access to home network 
...
Always in the beginning, AuthN lies outside home network, but
...
and there management of home network devices is allowed.
...
Home Network's future needs, for example  the change of authority and the
...
built. A dual-role model, which binds a smartphone to the home network and
...
With the proposed technique, provisioning of users at home networks would

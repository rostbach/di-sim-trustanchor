...
- access point, AP :: Connects wireless (Wi-Fi) clients to wired network.
...
secret keys are used for authenticating an IMSI to a MNO and that
...
5. A CPE (AP) forwards the authentication to a MNO's RADIUS server using
   RADIUS protocol.
6. The MNO has RADIUS server which uses a HLR-AuC for authentication
   ...
   the end. The MNO also asks (AuthZ phase) from the Cloud, whether IMSI user has an admin role.
 The MNO returns in a RADIUS message either /ACCESS-ACCEPT/, if user
   is both known AND has admin role,  or /ACCESS-REJECT/, if either
   property fails.
7. The CPE receives this ACCEPT or REJECT. If there were other RADIUSes
   between the CPE and the MNO, they would have acted
   as proxy RADIUS servers.
8. IF ACCEPTed, then the smartphone is both authenticated and
   authorized and it now 
   can send configuration change messages to the CPE, which recognizes them
   coming from an authorized  network.

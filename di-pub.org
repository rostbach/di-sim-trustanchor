...
#+LATEX_HEADER: \thesistype{draft-8.10.2015 Master of Science thesis}
...
would change the balance of authors or roles,
# (if role-based authorization in use), 
it needs to check that the change is permitted.  Permission 
needs to be asked from a trusted point, here mobile SIM. Instead of
...
method. The second would not involve smartphone at all but depended on earlier set
trust. The remaining two would depend on more complex setups involving tokens.


1) Changes are delivered from cloud to smartphone, which forwards them
   to devices located at management net after the
   authentication has succeeded.

2) Changes are delivered normally from cloud to CPE (CPEs) without
...
Using tickets has a connection to earlier studies on homenets.  In
Behringers work-in-progress
bootstrapping\cite{draft-behringer-bootstrap}, AuthZ happens likewise
first at cloud provider's end, but after checking device's Vendor
certificates, cloud provider gives the device a ticket of
authorization similar to Kerberos, which is based on Needham-Schroeder
protocol. Device presents that ticket to CPE which finally can
decide, whether it allows change.  



Instead, in our work the Authentication server can be an external
...
initial bootstrap as well as change of admin roles and some dangerous
combination of commands would need AA with the smartphone.


Bootstrapping protocols in general are used to bring the first trust anchor in an
environment and use that anchor to attach other devices to the same trust
circle.
...
# another
Another problem using in-flight addition of the authorization in RADIUS 
ACCESS-ACCEPT message(\cite{mail-huhtanen-2014} is mapping of 
authentication messages of IMSI and TMSI identitys.
# #+BEGIN_QUOTE

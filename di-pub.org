...
Homenet has 5 tasks to solve at home networks: service discovery, network security, 
...
It is well known, that the usability of the Kiosk-mode Wi-Fi
 networks is burden, because a user needs to go through 
...
their data access between cellular and  Wi-Fi when 
...
HS2.0 goals were to smooth roaming between Wi-Fi and 3GPP/LTE networks
and bring operator-grade to Wi-Fi by putting control in operators side. More
than offloading traffic, plans are to bring other services also to Wi-Fi.\cite{er-seamless}

In Hotspot 2.0, the cellular network may signal the smartphone to
switch to Wi-Fi. The smartphone then tries to find HS2.0 capable
access point and continue using Wi-Fi instead of cellular network.
In similar way, the smartphone could receive signal from the cellular
...
HS2.0. If it can, then the cellular network continues and orders the 
...
the put it to the management network.
...
miss the user decision part. The user, not the operator,
 must to give his consent to access the management network.
...
2. Cloud send the changes to CPE (2).
3. If the changes are privileged, they need to be approved by the phone user.
   The changes are sent also to the phone(3) and the user must
   authenticate  itself to the management network.
4. Phone user starts authentication process to management
   network using EAP-SIM and reveals its IMSI(4).
5. CPE  (AP) forwards authentication to MNO's RADIUS server using
   RADIUS protocol (5).
6. MNO have RADIUS server running and it authenticates IMSI user with
   its HLR-AuC (6).
 MNO also asks from the Cloud, whether IMSI user has an admin role (AuthZ). 
...
[ alternative presentation of flow, II: textual paragraph. Which one is better? ] 
...
using RADIUS protocol (5).  MNO have RADIUS server running and it
...
Wireless capture between WPA2-Supplicant and AP was made at
WPA2-Supplicant, before it left the wireless card. Capture was
not made in monitoring mode, so all 802.11 details in
data packets were not captured\cite{wireshark-capture}.
...
2. Packet capture of successful SIM-authentication with corresponding

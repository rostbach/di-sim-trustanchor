...
CPE wants to verify, if the changes are authorized. This thesis uses
the model where the management and
therefore the changes are allowed, if the smartphone user is currently logged in to the management network.

...
\item[GBA] Generic Bootstrapping Architecture, 3GPP standard for user authentication with help of shared key from operator
...
As can be seen, no public key infrastructure (PKI) is not used before setup
phase is ready.
...
It benefits from MNO's or any other identity centre's  knowledge of user's SIM.

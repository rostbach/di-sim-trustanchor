...
\thesistype{draft-30.10b.2015 Master of Science thesis}
...
Jolla is a new Finnish smartphone, running on ARMv7 processor, having its
root's on Nokia's last mobilephone line that used Linux
Distribution MeeGo in beginning of 2010's.
...
machine running under Virtualbox,
because it provided not only simulated Jolla environment on x86 
architecture,  but also a cross compiling environment Scratchbox, that
makes
 porting programs to Armv7 processor easy.
...
that even SHA1 signatures can be forged.
...
advertisement of mana\-gement network SSID. That is called ``network
...
\section{Usability of access points}
\label{sec-6-4}

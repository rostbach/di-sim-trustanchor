...
the configured network device. 
# , and if physical precence would 
be away 
#  What then, when the person is not
# physically present, but tries to connect remotely?

If external help were present, home owner would need more control
control in allowing only authorized operators in, because earlier used

...
is not used here, but would be third different possible factor.
...
the network  used is IP network instead of mobile phone network.
...
fully explaining our chosen method, we introduce some 
...
between the smartphone (local controller)

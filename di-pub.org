...
encryption between the smartphone and AP by negotiating session keys
after 802.1X has opened the virtual port in AP for the smartphone.

 WPA was an early subset of then upcoming 802.11i standard,
...
It does not include AuthZ information, which is RADIUS's responsibility
or session keys, which are negotiated by WPA.
d
# or session keys.  needed in the following message transfer. Of
# those, RADIUS is responsible of delivering AuthZ (and also
# encapsulated AuthN) to the Authenticator (AP) and WPA is used to
...
unique IMSI (International Mobile Subcriber Identity).  SIM card usage
...
sequencenumbering preventing replay attacks and its digests use SHA-256
...
set, IMSI may be left out and instead a temporal IMSI (TMSI) can be used
to hide client's identity. It must be noted, that this TMSI differs from
TMSI used in 3GPP network. Those context must not be mixed, otherwise
the security they bring may lose its power.
...
#+begin_src ditaa :file eap-sim-simple.png :cmdline -E -r -s 4
  Mobile phone           Authenticator (AP)          Authentication server      MNO(HLR_AuC)
      |      EAP TRAFFIC      |                                  |                   |
      +<--------------------->|                                  |                   |
      |                       |  RADIUS (EAP)                    |                   |
      |                       +--------------------------------->|                   |
      ...
      |                       |                                  |
+-----+--------------------+  |                                  |    
|verify MNO, derive session|  |                                  |    
|keys, decrypt pseudonym,  |  |                                  |    
|digest challenge          |  |                                  |    
+-----+--------------------+  |                                  |    
      |                       |                                  |
      | Response              |                                  |
      ...
      |                       |                                  |
#+end_src
#+CAPTION: Detailed EAP-SIM simplified sequence diagram, source RFC4186
...
communication, but the keys need to be delivered through an trusted
channel, and so it can be seen that trust really is the first layer to
be fixed.
...
been done already earlier. For example, TR-069 
# lähde ehkä ei ihan kerro TR-069 perusasioita..
standard\cite{iptvtr069} for CPEs such as ADSL broadband routers or
...
`smart'. Most smartphones have Wi-Fi capabilities and so are
...
base having smartphones is enormous. The users are located in operators' user databases
...
creation occurs (WPA session) between AP and the smartphone. 
...
#+CAPTION: Cisco's view of 802.1x auth [To be removed from final]
...
Figure[[fig:cisco-802.1x]] reminds us, how 802.1X works.

...
5. A CPE (AP) forwards the authentication to a MNO's RADIUS server
...
In second scenario (Figure[[fig:scenario-II]]), AuthN is asked from an MNO but
AuthZ is checked from a local database. Local data comes from a data
model, i.e., from configuration data and will be saved in CPE, or some other

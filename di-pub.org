...
*** to somewhere else, is not trust property
...
The
fault lies there, that GSM specification does not require the mobile network to
authenticate itself to the phone and thus GSM allows man in the middle
attack device called IMSI-catcher to fake as being a base station.
When the smartphone tries to attach to a fake base station, the smartphone reveals its

...
#+LATEX_HEADER: \thesistype{Master of Science thesis}
...
signature application (see Section [[sec:altmethods]]),
...
the Internet. This chapter presents  the homenet working group
...
* Design of home network trust anchor and change management
:PROPERTIES:
...
# TÄHÄN 1 radius
If RADIUS AVPs do not suffice, there are also special Vendor Specified
...
Besides VSAs, RADIUS servers may implement different vocabulary in their AVP set.
...
# AA now done. How about change propagation? section here with disconnects?
** Management design
...
located in diverse places, as can be seen in Section [[#sec:scenarios]].
# ### 
...
# solution for missing internet connection is 
...
Table[[table-scenarios]] presents the non-inclusive list of locations of
...
8. If ACCEPTed,  the smartphone is both authenticated and
...
#+end_src
#+CAPTION: Scenario IV with offline AA 
...
# howto describe the correct form of -
...
2. Smartphone uses telnet (or ssh) to login to the ROUTER
   with credentials.
3. ROUTER ( as RADIUS client) checks AA from Authentication Server (or 
   proxy).
4. AA-server answers based on earlier SIM-authentication that this
   request is correct.
...
RADIUS, whenthat is possible as was mentioned in Section [[sec:radius]] (RADIUS).
...
** EAP-SIM authentication testbed
...
a RADIUS server. From those, executable binary programs  wpa_supplicant, hostapd (for wired RADIUS
...
the phone (message 7 in Figure[[fig:eap-sim-radius]]), but the RADIUS server
...
The tests were run from a shell program (Appendix [[app:fulleap]]), which
...
# We might notice the difference on used 802.1X versions: AP uses
# version
...
When we remember from Section [[sec:sim-based-auth]], that IMSI can be
...
In  the WPA2-Supplicant configuration file (see Appendix [[app:wpa-conf]]) both the identity and
...
Appendix [[app:hlraucgw]]. There one can see the simulated Milenage parameters including SIM secrets.
...
# The OS is called Sailfish and it includes user
...
Jolla's base  WPA-supplicant (wpa_supplicant) belongs to Mer core software, and it was here re-built for
...
To compile wpa_supplicant with EAP-SIM simulator, a PCSC component was
needed. So first pcsc-lite was downloaded and cross-compiled in
MerSDK. Then wpa_supplicant was compiled with correct parameters in
the same place. Finally, the wpa_supplicant was stripped of debugging
...
In the following example, file /mersdk/ represent the private SSH key of type RSA.
#+BEGIN_EXAMPLE
# ssh -l mersdk -p 2222 \
...
#+END_EXAMPLE
The wpa_supplicant was compiled in a similar way. The needed options were
...
#+END_EXAMPLE
Finally, the program wpa_supplicant-pcsc and a corresponding
...
*** Running the new binary on a real smartphone
...
Because the /libpcsclite/ libraries are in a non-standard
place (in /usr/local/lib), we had to start wpa_supplicant with the
...
# let us
# discuss some of the results and findings. We will
...
As discussed in Section [[sec:sim-based-auth]],  IMSI is sent in clear 
...
The TMSI can even be re-changed using re-authentication as shown in Figure[[fig:tmsi]].
...
The integrity issues were partly handled in Section [[sec:radius-macs]] describing
...
as a secure RADIUS proxy such as /radproxy/ \cite{uninett-radproxy}.
...
a smartphone, the needed components are /pcsc-lite/ for accessing the SIM card, /wpa_supplicant/ for
WPA2 client, and possibly used connection manager (/connman/,
/wicd/, or /Network Manager/ in Linux). This is in line with what was done in testing without /pcsc-lite/,
...
# [*TBDfinal Some part to be moved to the Conclusion chapter*]
...
The term /provisioning/ can mean adding users to the home network with
the correct
...
[[#cha:design]], where there is a local authentication server in home network.

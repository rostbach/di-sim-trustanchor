...
When this is compared with Figure[[fig:eap-sim-radius]], we see that we
got no further than message 7, where a nonce, selected version, and
an identity were sent from the phone, but RADIUS server did not know how to handle them. 

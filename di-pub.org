...
#+LATEX_HEADER: \thesistype{draft-2b.11.2015 Master of Science thesis}
...
Local controller   AP               Managed device (Router)
     |             |                         |
     | 1. SSH login|                         |
     |------------>+      2.SSH login on wire|
     |             |------------------------>+
     ...
     |                                       |
#+end_src
#+CAPTION: Challenging trust from a managed device
...
#+END_LaTeX
** Shell, that starts programs and logs
<<app:fulleap>>
:PROPERTIES:
:NAME: app:fulleap
:END: 

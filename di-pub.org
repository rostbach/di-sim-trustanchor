...
role.  Once user has been authorized into a management network, access
...
default network, i.e., it just gives basic access. As for other
provisioning parameters, not all end devices support them.

In the first prototype it is enough to identify an authorized
smartphone's SIM.  The smartphone holding the SIM is granted the access to
